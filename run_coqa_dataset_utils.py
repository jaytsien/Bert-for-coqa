...
def read_coqa_examples(input_file, history_len=2, add_QA_tag=False):
    ...
    for data_idx in tqdm(range(len(input_data)), desc='Generating examples'):
        datum = input_data[data_idx] # 整篇文章
        ...
        for i in range(len(datum['questions'])):
            ...
            for j in range(i - history_len, i + 1):
                ...
                long_question += (' <Q> ' if add_QA_tag else
                                  ' ') + datum['questions'][j]['input_text'] # 取出第i个问题
                if j < i:
                    long_question += (' <A> ' if add_QA_tag else
                                      ' ') + datum['answers'][j]['input_text'] # 第i个问题的
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(tqdm(examples, desc="Generating features")):
        ...
        # query_tokens = BertTokenizer.tokenize(example.question_text)
        ...
        if len(query_tokens) > max_query_length: # ['when', 'was', 'the', 'va', '##t', 'formally', 'opened', '?'] > 64
        ...
        for (i, token) in enumerate(example.doc_tokens):
            orig_to_tok_index.append(len(all_doc_tokens)) # [0,1,2,...,]
        ...
        if cls_idx < 3:
            ...
            if example.end_position < len(example.doc_tokens) - 1: # 判断答案是否超出正文的内容
                tok_end_position = orig_to_tok_index[example.end_position +
                                                     1] - 1
            else:
                tok_end_position = len(all_doc_tokens) - 1
            (tok_start_position, tok_end_position) = _improve_answer_span( # 正文内容，回答问题的起始地址，终止位置
                all_doc_tokens, tok_start_position, tok_end_position, tokenizer,
                example.orig_answer_text)
        # The -3 accounts for [CLS], [SEP] and [SEP]
        max_tokens_for_doc = max_seq_length - len(query_tokens) - 3 # 正文允许的最长长度
        ...
        doc_spans = []
        start_offset = 0
        while start_offset < len(all_doc_tokens): # all_doc_tokens 正文内容
            length = len(all_doc_tokens) - start_offset 
            if length > max_tokens_for_doc: # 当正文内容 大于 允许的最长长度，重新赋值
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            for i in range(doc_span.length):
                ...
                token_to_orig_map[len(tokens)] = tok_to_orig_index[split_token_index] # tok_to_org_index [0,257] 正文长度

                '''
                tokens:['[CLS]', 问题, '[SEP]', 正文, '[SEP]']
                token_to_orig_map,意思就是正文的序号排在问题的后面
                '''
            ...
            if cls_idx >= 3:
                ...
                doc_start = doc_span.start # 0
                doc_end = doc_span.start + doc_span.length - 1 # 277
                out_of_span = False
                if not (tok_start_position >= doc_start   # 答案的起始位置 文章的起始位置  答案的内容终止位置小于文章的末尾
